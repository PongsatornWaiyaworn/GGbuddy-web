      ...
      try {
        ...
        chatGroups.forEach((group) => {
          const ws = new WebSocket(
            `${API_BASE_URL_SOCKET}/ws?username=${username}&group_id=${group.id}`
          );
          ...
          ws.onmessage = async (event) => {
            try {
              const blockedRes = await fetch(`${API_BASE_URL}/blocked-list/${username}`, {
                method: "GET",
                headers: {
                  Authorization: `Bearer ${token}`,
                  "Content-Type": "application/json",
              ...
              let blockedUsernames = null
              let isBlocked = false
              if(blockedData){
                blockedUsernames = blockedData.map((entry) => entry.BlockedUsername);
                isBlocked = blockedUsernames.includes(data.sender_id);
              }
                  ...
                  (t) => (
                    ...
                    >
                      <div className="font-semibold text-sm text-gray-800">
                        ข้อความใหม่จาก{" "}
                        <span className="text-blue-600">{data.sender_id}</span> ใน {group.name}
                      </div>
